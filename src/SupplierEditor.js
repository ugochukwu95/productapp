...
export class SupplierEditor extends Component {
	constructor(props) {
		...
		this.state = {
			formData: {
				...
				products: props.supplier.products != null ? props.supplier.products.map(p => p.id) : [],
			}
		}
	}
	...
	handleChange = (ev) => { 
		ev.persist();
		this.setState(state => state.formData[ev.target.name] = 
	...
	render() {
		return <div className="m-2">
			...
			<div className="form-group">
				<label>City</label>
				<input className="form-control" name="city" value={ this.state.formData.city }
					onChange={ this.handleChange } />
			</div>
			<div className="form-group">
				<label>Products</label>
				<input className="form-control" name="products" value={ this.state.formData.products }
					onChange={ this.handleChange } />
			</div>
